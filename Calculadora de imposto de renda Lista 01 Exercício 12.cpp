...
int main(){
	...
	if (imposto1<0||imposto2<0||imposto3<0||imposto4<0)//Condição que isenta o contribuinte do imposto caso o numero de dependentes proporcione um desconto superior ao percentual de renda designado pelo imposto 
		printf("Você está isento do imposto de renda pelo alto número de dependentes");
	else if (rendamensal<=2*salariomin)//Condição que isenta os contribuintes com renda inferior a 2 salários mínimos
